...
`;At(Ot,Je);function ae(t,i,e){if(e.style.height&&(e.dataset.fixedHeight="1"),(e.style.height||e.style.maxHeight)&&(t&&(t.style.height=e.style.height,t.style.maxHeight=e.style.maxHeight,t.style.overflowY="scroll",t.style.borderTopRightRadius="0",t.style.borderBottomRightRadius="0"),e.style.borderTopRightRadius="0",e.style.borderBottomRightRadius="0",e.style.overflowY="scroll"),!t||!i)return;let n=window.getComputedStyle(t),s=window.getComputedStyle(i),o=window.getComputedStyle(e),r=t.offsetWidth-t.clientWidth;t.dataset.defaultPadding||(t.dataset.defaultPadding=n.paddingTop),e.dataset.defaultPadding||(e.dataset.defaultPadding=o.paddingTop),i.style.top=`${n.borderTopWidth}`,i.style.left=`${n.borderLeftWidth}`,i.style.right=`${parseInt(n.borderRightWidth)+r}px`,!e.style.height&&!e.style.maxHeight&&(i.style.borderTopRightRadius=`${o.borderTopRightRadius}`),i.style.borderTopLeftRadius=`${o.borderTopLeftRadius}`,t.style.paddingTop=`calc(${t.dataset.defaultPadding} + ${s.height})`,e.style.paddingTop=`calc(${e.dataset.defaultPadding} + ${s.height})`}var re=new ResizeObserver(t=>{for(let i of t){let e=i.target,n=e.offsetWidth.toString();if(n!=e.dataset.width&&(e.dataset.width=n,i.contentBoxSize)){let s=M("textarea",null,e),o=M("div",".squire-input",e),r=M("div",".squire-toolbar",e);ae(o,r,s)}}}),mt=class extends ne{get el(){return M("textarea",null,this)}created(){if(!this.el){let e=R("textarea",{name:this.getAttribute("name"),value:this.getAttribute("value")});ut()&&e.classList.add("form-control"),this.appendChild(e)}this.classList.add("squire-editor");let i=this.el;i.setAttribute("hidden",""),i.style.overflow="hidden",i.spellcheck=!1,this.init()}async init(){await se(oe);let i=M("div",".squire-toolbar",this),e=M("div",".squire-input",this);if(i&&i.remove(),e&&e.remove(),!this.dataset.toolbar){i=R("div"),i.classList.add("squire-toolbar"),this.appendChild(i);let a=[[{name:"bold",action:"bold",format:"b",removeAction:"removeBold",icon:`<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          ...
          </svg>`},{name:"image",action:"insertImage",prompt:!0,icon:`<svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
          ...
          </svg>`}]],l=this.dataset.buttons?this.dataset.buttons.split(","):[],c=(d,h=i)=>{if(l.length&&!l.includes(d.name))return;let f=R("button");f.type="button",f.innerHTML=d.icon,f.dataset.action=d.action,f.dataset.removeAction=d.removeAction||"",f.dataset.format=d.format||"",f.dataset.prompt=d.prompt||"",f.ariaLabel=d.name,h.appendChild(f)};a.forEach(d=>{if(Array.isArray(d)){let h=R("div");h.classList.add("squire-toolbar-group"),i.appendChild(h),d.forEach(f=>c(f,h))}else c(d)})}e=R("div"),e.classList.add("squire-input"),ut()&&e.classList.add("form-control"),this.appendChild(e);let s=this.el;s.style.fontFamily="ui-monospace, 'Cascadia Code', 'Source Code Pro', Menlo, Consolas, 'DejaVu Sans Mono', monospace",ae(e,i,s),this.config=Object.assign({blockTag:"div",blockAttributes:{class:"paragraph"},tagAttributes:{blockquote:{class:"blockquote"}},sanitizeToDOMFragment:(a,l)=>{let c=R("div");yt(c,a,oe);let d=kt(c.innerHTML);return d?document.importNode(d,!0):document.createDocumentFragment()}},this.config);let o=this.dataset.allowPaste,r=this.dataset.pasteSize||1920;this.squire=new ie(e,this.config),this.squire.setHTML(s.value),this.squire.addEventListener("input",()=>{let a=this.squire.getHTML();this.el.value=a}),this.squire.addEventListener("cursor",()=>{let a=this.squire.getCursorPosition();if(!a)return;let l=e.getBoundingClientRect().top,c=a.top-l,d=a.bottom-l,h=0,f=0,u=e.offsetHeight,m=16;c<f+m?h=c-f-m:d>u-m&&(h=d+m-u),h&&e.scrollBy(0,Math.round(h))}),o&&this.squire.addEventListener("pasteImage",a=>{let c=[...a.detail.clipboardData.items].filter(h=>/image/.test(h.type));if(!c.length)return!1;let d=new FileReader;d.onload=h=>{let f=""+h.target.result,u=this.squire;var m=new Image;m.src=f,m.onload=function(){if(this.width>r||this.height>r){alert(`Your images need to be less than ${r} pixels in height and width.`);return}u.insertImage(f)}},d.readAsDataURL(c[0].getAsFile())})}connected(){this.addEventListener("click",this),this.addEventListener("input",this),re.observe(this)}disconnected(){this.removeEventListener("click",this),this.removeEventListener("input",this),re.unobserve(this)}handleEvent(i){this[`_${i.type}`](i)}_input(i){let e=this.el;i.target===e&&!e.dataset.fixedHeight&&(e.style.height="0",e.style.height=e.scrollHeight+"px")}_click(i){let e=i.target.closest("button");if(!e)return;let n=this.squire,s=e.dataset.action,o=e.dataset.removeAction,r=e.dataset.format,a=e.dataset.prompt,l,c,d=M("div",".squire-input",this);if(s==="html"){let h=M("textarea",null,this);if(h.hasAttribute("hidden")){if(!h.dataset.fixedHeight){let f=Math.max(parseInt(window.getComputedStyle(d).height),h.scrollHeight);h.style.height=f+"px"}d.setAttribute("hidden",""),h.removeAttribute("hidden")}else this.squire.setHTML(h.value),h.setAttribute("hidden",""),d.removeAttribute("hidden");return}if(s&&n&&n[s]){if(d.hasAttribute("hidden"))return;let h=n.getSelection();if(r&&o&&n.hasFormat(r))o==="changeFormat"&&(l=null,c={tag:r}),n[o](l,c);else{if(a){if(h.collapsed&&r)return;if(l=window.prompt(),!l){n[o](l,c);return}}s==="changeFormat"&&(l={tag:r}),n[s](l,c)}}}destroyed(){this.squire.destroy(),this.squire=null}};customElements.get(Ot)||customElements.define(Ot,mt);var xi=mt;})();
