...
class FlatpickrInput extends FormidableElement {
	...
	created() {
		...
		const onNavigate =
			typeof this.config.onNavigate === "string"
				? getGlobalFn(this.config.onNavigate)
		...
		const originalValue = this.hiddenInput
	...
	async loadConfig(instance) {
		...
		const params = {
			v: instance.selectedDates.map((v) => toDate(v)),
			year: instance.currentYear,
			month: instance.currentMonth,
		};
		const abortController = new AbortController();
		const configData = await fetchJson(this.config.configUrl, params, {
	 ...
	 */
	getDaySpan(date) {
		const d = this.flatpickr.formatDate(
			date,
			this.config.ariaDateFormat || "F j, Y",
		);
